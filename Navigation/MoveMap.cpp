...
namespace MMAP
{
	...
	bool MMapManager::loadMap(unsigned int mapId, int x, int y)
	{
		...
		// Reject tiles generated by third-party extractors. The watermark is
		// baked into the struct at extraction time; absence = tile not ours.
		if (file_read != 1 || fileHeader.mmapMagic != MMAP_MAGIC ||
			fileHeader.watermark != MMAP_WATERMARK)
		{
			return false;
		}
		...
		if (!file)
			return false;
		fseek(file, sizeof(MmapTileHeader), SEEK_SET);
